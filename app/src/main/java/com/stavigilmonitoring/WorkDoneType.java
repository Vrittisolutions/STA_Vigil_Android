...
import com.adapters.WorkDoneTypeAdp;
...
public class WorkDoneType extends Activity {
	...
	private ArrayList<WorkType> searchResults;
	private static DownloadWorkType asynwork;
	WorkDoneTypeAdp doneTypeAdp;
	DatabaseHandler db;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(com.stavigilmonitoring.R.layout.selecttypetime);
		searchResults = new ArrayList<WorkType>();
		...
		if (dbvalue()) {
			...
			try{
				ut.showD(WorkDoneType.this, "nonet");
			}catch (Exception e){
				e.printStackTrace();
			}
		}
	...
	public class DownloadWorkType extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			String urlnet = "http://beta.ekatm.com/vwb/webservice/VWBservice.asmx/GetActivityList?Caller_no="+mobno;
			...
			try {
				...
				if (responsemsg.contains("<ActivityId>")) {
					sop = "valid";
					//sql.execSQL("DROP TABLE IF EXISTS ActivityAllocationMaster");
					//sql.execSQL(ut.getActivityAllocationMaster());
					sql.delete("ActivityAllocationMaster",null,null);

					Cursor cur1 = sql.rawQuery("SELECT * FROM ActivityAllocationMaster", null);
					...
					for (int i = 0; i < NL.getLength(); i++) {
						...
						long ad = sql.insert("ActivityAllocationMaster", null, values2);
					}
	...
	//	DatabaseHandler db = new DatabaseHandler(this);
		SQLiteDatabase sql = db.getWritableDatabase();
		int count = 0;
		Cursor c = sql.rawQuery("SELECT ActivityId,ActivityName FROM ActivityAllocationMaster order by ActivityName", null);
		if (c.getCount() > 0) {
			c.moveToFirst();
			do {

				String Type = c.getString(c.getColumnIndex("ActivityId"));
				String Type1 = c.getString(c.getColumnIndex("ActivityName"));
				WorkType bean = new WorkType();
				bean.setWorktype(Type1);
				bean.setWorkid(Type);
		...
		doneTypeAdp = new WorkDoneTypeAdp(WorkDoneType.this, searchResults);
	...
	private boolean dbvalue() {
		...
		try {
			...
			SQLiteDatabase sql = db.getWritableDatabase();
			Cursor cursor = sql.rawQuery("SELECT * FROM ActivityAllocationMaster", null);// SoundLevel_new
