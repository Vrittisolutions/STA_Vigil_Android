...
public class SEActivitiesHistoryActivity extends AppCompatActivity {
    ...
    DatabaseHandler db;
    SQLiteDatabase sql;
    String StationName,mobno,network,subnetwork,IssuedToName,Activity=null,SubType1,Type1,frompage1,actIssuedToPersonName, userName;
    ...
    public void init(){
        parent = SEActivitiesHistoryActivity.this;
    ...
    public void setListeners(){
        listhistory.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intent = new Intent(SEActivitiesHistoryActivity.this,SE_StationwiseHistory_Activity.class);
                intent.putExtra("IssuedUserName",list.get(position).GetIssuedToName());
    ...
    private void updatelist() {
        ...
        Cursor c = sql.rawQuery(
                "SELECT ActivityName, ActualStartDate, ActualEndDate, Status, StationName, ActivityId, UserName,UserMasterId,IssuedUserName," +
                        ...
                        + userName + "' AND ActivityName like '%Video recording for%' ORDER BY ActualEndDate", null);
        if (c.getCount() > 0) {
            c.moveToFirst();
            do {
