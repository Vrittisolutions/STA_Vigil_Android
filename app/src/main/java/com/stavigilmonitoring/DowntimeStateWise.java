...
import com.adapters.DowntimeAdapt;
...
public class DowntimeStateWise extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lstcsn.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent i = new Intent(DowntimeStateWise.this,
						DowntimeMain.class);
		...
		protected String doInBackground(String... params) {
			...
			try {
				...
				if (responsemsg.contains("<NetworkCode>")) {
					...
					if (responsemsg.contains("<StationDownTime>")) {
						sop = "valid";
						...
						Cursor cur1 = sql.rawQuery("SELECT * FROM Downtime",
								null);
						...
						for (int i = 0; i < nl2.getLength(); i++) {
							...
							sql.insert("Downtime", null, values2);
						}
	...
	protected void showD(String string) {
		...
		final Dialog myDialog = new Dialog(DowntimeStateWise.this);
	...
	private void updatelist() {
		...
		if (c.getCount() > 0) {
			...
			do {
				...
				Cursor c1 = sql
						.rawQuery(
								"Select distinct InstallationDesc from Downtime c1 inner join AllStation c2 on c1.InstalationId=c2.InstallationId where c2.NetworkCode='"
										+ Type + "'", null);
				count = c1.getCount();
				/*
		...
		lstcsn.setAdapter(new DowntimeAdapt(DowntimeStateWise.this,
				searchResults));
