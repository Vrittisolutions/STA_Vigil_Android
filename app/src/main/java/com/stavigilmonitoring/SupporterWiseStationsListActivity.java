...
public class SupporterWiseStationsListActivity extends Activity {
	...
	String resposmsg,sop,type,mobno,SubNW = "";
	int scount = 0;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		txtheader =  findViewById(R.id.onactivitynamereassign);
		...
		mAllnet.setVisibility(View.GONE);
		...
		SubNW = i.getStringExtra("SubNW");
		mText.setText(type+"-All");

		txtheader.setText("Stations List - "+type +" - "+SubNW);
				...
				// TODO Auto-generated method stub
				      if (ut.isnet(getApplicationContext())) {
						fetchdata();
				      	...
				      	try{
							ut.showD(SupporterWiseStationsListActivity.this,"nonet");
						}catch (Exception e){
				      		e.printStackTrace();
						}
					}
		...
		mList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				intent.putExtra("InstallationId", mSearchlist.get(position).getInstallationId());
				intent.putExtra("InstallationName", mSearchlist.get(position).getStatioName());
				setResult(STATION, intent);
				finish();
	...
	public void getDetail() {
		...
		Cursor c1 = sql.rawQuery("Select * from ConnectionStatusFiltermob WHERE SubNetworkCode='"+SubNW+"'ORDER BY InstallationDesc ASC",null);
		...
		listAdapter = new GetPassAdapt(this, mSearchlist);
	...
	mSearchlist.clear();
		...
		Cursor c = sql.rawQuery(
						"SELECT DISTINCT SubNetworkCode FROM ConnectionStatusFiltermob where NetworkCode='"
								+ type + "' AND SubNetworkCode='"+SubNW+"' ORDER BY SubNetworkCode",
						null);
		// ,InstallationId
		if (c.getCount() > 0) {
			c.moveToFirst();
			do {
			 ...
			 Cursor c1 = sql.rawQuery("SELECT DISTINCT InstallationDesc,InstalationId FROM ConnectionStatusFiltermob WHERE SubNetworkCode='"+SubNetworkCode+"'",null);
             count = c1.getCount();
             	String StatioName = c1.getString(c1.getColumnIndex("InstallationDesc"));
             	String StatioId = c1.getString(c1.getColumnIndex("InstalationId"));
				// Type = Type.replaceAll("0", "");
				// Type = Type.replaceAll("1", "");
				// if (!Type.trim().equalsIgnoreCase("")) {
				StateList sitem = new StateList();
                sitem.SetNetworkCode(SubNetworkCode);
                sitem.setStatioName(StatioName);
                sitem.setInstallationId(StatioId);
		...
		StationEnquiryfilteradp adp  = new StationEnquiryfilteradp(SupporterWiseStationsListActivity.this, mSearchlist);
		...
		protected void onPostExecute(String result) {
			...
			try {
				if (sop.equals("valid")) {
					...
					try{
						ut.showD(SupporterWiseStationsListActivity.this, "invalid");
					}catch (Exception e){
						e.printStackTrace();
					}
				}
