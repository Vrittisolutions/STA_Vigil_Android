...
public class AlrtsStnListAll extends Activity{
					...
					do{
						...
						try {
							...
							/*if (diffDays == 0 && diffHours == 0
								...
								diffstr = "";
								if (diffDays == 0 && diffHours == 0){	diffstr = diffMinutes + "Min";  }
								else if (diffDays == 0){ 	diffstr = diffHours + "hr"; }
								else {
									if (diffDays >= 32) {
										long yc = diffDays / 30;
										if (yc >= 12){ diffstr = (yc / 12) + " Year";}
										else{ diffstr = yc + " Month";}
									} else{		diffstr = diffDays + "days ";		}
	...
	private void updatelist2(){
		...
		Cursor c = sql
				.rawQuery(
						"Select distinct InstallationDesc,InstalationId,AlertId from AlrtCountTable where cnt != '0' and NetworkCode='"+mType+"' Order by CAST(AlertId AS INT)",
						//+ " Order by NetworkCode",
						null);
		if (c.getCount()>0){
			Log.e("MY c COUNT", String.valueOf(c.getCount()));
	...
	public void updatelist3(String inId) {
		// TODO Auto-generated method stub
		//DatabaseHandler db = new DatabaseHandler(AlrtsStnListAll.this);
		SQLiteDatabase sql = db.getWritableDatabase();
	...
	protected void AlertDetails(Bundle dataBundle2) {
		...
		startActivity(intent);	*/	
		Intent intent = new Intent(AlrtsStnListAll.this,
