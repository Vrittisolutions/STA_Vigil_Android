...
public class BackgroundPlaylist extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		((TextView) findViewById(com.stavigilmonitoring.R.id.onactivitynamereassign))
				.setText("Background Playlist -" + subType);
		...
		db = new DatabaseHandler(BackgroundPlaylist.this);
	...
	private void updatelist() {
		...
		Cursor c = sql
				.rawQuery(
						"SELECT DISTINCT  InstalationId,InstallationDesc FROM Backgroundplaylist where SubNetworkCode='"
								+ subType + "' ORDER BY InstallationDesc", null);
		...
		listAdapter = new BackgroundPlayAdp(BackgroundPlaylist.this, list_data);
	...
	private boolean dbvalue() {
		try {
			...
			Cursor cursor = sql.rawQuery(
					"SELECT *   FROM Backgroundplaylist WHERE SubNetworkCode='"
							+ subType + "'", null);
		...
		protected void onPostExecute(String result) {
			...
			try {
				...
				if (sop.equals("valid")) {
					...
					try{
						ut.showD(BackgroundPlaylist.this, "NoPlay");
					...
					try{
						ut.showD(BackgroundPlaylist.this, "ServerError");
					}catch (Exception e){
						e.printStackTrace();
					}
				} else {
	...
	protected void showD(String string) {
		...
		final Dialog myDialog = new Dialog(BackgroundPlaylist.this);
