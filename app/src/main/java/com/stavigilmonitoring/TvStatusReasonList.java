...
public class TvStatusReasonList extends Activity {
	ListView list;
	ImageView iv;
	TextView tvpwd;
	...
	private boolean getdbvalue() {
		try {
			...
			Cursor cursor1 = sql
					.rawQuery("SELECT * FROM  TVStatusReason", null);
	...
	protected void showD(String string) {
		...
		final Dialog myDialog = new Dialog(TvStatusReasonList.this);
	...
	public ArrayList<String> getDetail() {
		...
		SQLiteDatabase sql = db.getWritableDatabase();
		Cursor c1 = sql.rawQuery("select * from TVStatusReason", null);
		...
		protected String doInBackground(String... params) {
			...
			try {
				...
				//	DatabaseHandler db = new DatabaseHandler(getBaseContext());
					...
					//sql.execSQL("DROP TABLE IF EXISTS TVStatusReason");
					System.out.println("------------- 3-- ");
					//sql.execSQL(ut.getTVStatusReason());
					sql.delete("TVStatusReason",null,null);

					System.out.println("------------- 4-- ");
					System.out.println("------------- 5-- ");

					Cursor c = sql.rawQuery("SELECT *   FROM TVStatusReason",
							null);
					...
					for (int i = 0; i < nl.getLength(); i++) {
						...
						sql.insert("TVStatusReason", null, values);
