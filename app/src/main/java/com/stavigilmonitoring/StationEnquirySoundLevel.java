...
import com.adapters.StationEnquirySoundAdp;
...
public class StationEnquirySoundLevel extends Activity {
	String Type;
	String mobno, link;
	private AsyncTask depattask;
    String sop = "no";
	...
	StationEnquirySoundAdp soundlevelAdapter;
	...
	String conn = "invalid";
	String finalDate, subType, mNet, mInstallation, mStation;
	DatabaseHandler db;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ut = new utility();
		mStation = intent.getStringExtra("stnname");
		mInstallation = intent.getStringExtra("stninst");
		mNet = intent.getStringExtra("Network");
		// Network
		title.setText(mStation + "-Sound Level");
		...
		if (dbvalue()) {
			...
			try{
				ut.showD(StationEnquirySoundLevel.this, "nonet");
			}catch (Exception e){
				e.printStackTrace();
			}
		}
		...
		sound.setOnItemClickListener(new OnItemClickListener() {
			...
			//	if (dbvalue()) {
					...
					/*if (soundlevelBeanslist.get(position)
							...
							.equalsIgnoreCase("No Recent Data Found")) {
						...
						dataBundle.putString("Installation",
								soundlevelBeanslist.get(position)
										.getInstalationid());

						Intent myIntent = new Intent();
						myIntent.setClass(getApplicationContext(),
								StationEnquirySoundLevelData.class);
	 ...
	 * 
	 * System.out.println("==========@#@# actid " + s); DatabaseHandler db1 =
	 * new DatabaseHandler(this); SQLiteDatabase db = db1.getWritableDatabase();
	 * String[] params = new String[1]; params[0] = s; Cursor c2 = db.rawQuery(
	 * "SELECT * FROM SoundLevel where StationName=? ", params); String stnname
	 * = "";
	 * 
	 * if (c2.getCount() == 0) { c2.close(); db.close(); db1.close();
	 * 
	 * } else {
	 * 
	 * c2.moveToFirst(); stnname =
	 * c2.getString(c2.getColumnIndex("StationName"));
	 * 
	 * c2.moveToLast();
	 * 
	 * c2.close(); db.close(); db1.close();
	 * 
	 * }
	 * 
	 ...
	 * stnname); // dataBundle.putString("ActivityName", ActivityName);
	 * 
	 * // finish(); Intent myIntent = new Intent();
	 * myIntent.setClass(getApplicationContext(), SoundLevel.class);
	 * 
	 * myIntent.putExtras(dataBundle); startActivity(myIntent); // finish(); //
	...
	private boolean dbvalue() {
		try {
			...
			Cursor cursor = sql.rawQuery(
					"Select * from SoundLevel_StnEnq s where s.InstalationId='"
							+ mInstallation + "'", null);
	...
	public void getDetail() {
		...
		Cursor c = sql
				.rawQuery(
						"Select Distinct s.InstallationDesc , s.CallibrationDate, s.AO , s.InstalationId ,s.ServerTime from SoundLevel_StnEnq s WHERE s.InstalationId='"
								+ mInstallation
								+ "' ORDER BY s.InstallationDesc ", null); // s.ServerTime,
		// Select DISTINCT s.InstallationDesc , s.CallibrationDate, s.AO ,
		// s.InstalationId ,s.ServerTime from SoundLevel_new s INNER JOIN
		// SoundLevel sl ON sl.InstallationId = s.InstalationId where
		// sl.NetworkCode='"
		// + Type + "' ORDER BY s.InstallationDesc
		Log.e("SoundLevel ", "" + c.getCount());
		if (c.getCount() == 0) {

			c.close();
			Toast.makeText(getApplicationContext(),
					"No Data available For " + mStation + " Station...",
			...
			do {
				...
				if (!(cbvalue.equalsIgnoreCase("No Recent Data Found"))) {
					if (cbvalue.length() > 0) {
		...
		soundlevelAdapter = new StationEnquirySoundAdp(getApplicationContext(),
				soundlevelBeanslist);
		// new StationEnquirySoundAdp(getApplicationContext(),
		// soundlevelBeanslist);
	...
	public class DownloadxmlsDataURL extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			try {
				...
				//sql.execSQL("DROP TABLE IF EXISTS SoundLevel_StnEnq");
				System.out.println("------------- 3-- ");
				//sql.execSQL(ut.getSoundLevel_StnEnq());
				sql.delete("SoundLevel_StnEnq",null,null);
				...
				if (responsemsg.contains("<InstalationId>")) {
					...
					Cursor c = sql.rawQuery("SELECT * FROM SoundLevel_StnEnq",
							null);
					...
					for (int i = 0; i < nl.getLength(); i++) {
						...
						for (int j = 0; j < c.getColumnCount(); j++) {
							...
							System.out.println("-------------column value"
									+ columnValue);
							/*
							 * // CallibrationDate=Oct 7 2015 7:05AM if
							 * (columnName.equalsIgnoreCase("CallibrationDate"))
							 * { try { Calendar cal = Calendar.getInstance(); //
							 * "MM/dd/yyyy hh:mm:ss aa" SimpleDateFormat format
							 * = new SimpleDateFormat( "MMM dd yyyy hh:mm");
							 * 
							 * Date Startdate = format.parse(columnValue); Date
							 * Enddate = cal.getTime(); long diff =
							 * Enddate.getTime() - Startdate.getTime(); long
							 * diffSeconds = diff / 1000 % 60; long diffMinutes
							 * = diff / (60 * 1000) % 60; long diffHours = diff
							 * / (60 * 60 * 1000) % 24; long diffDays = diff /
							 * (24 * 60 * 60 * 1000);
							 * 
							 ...
							 * diffMinutes <= 30) {
							 * 
							 * } else { conn = "valid"; } } catch (Exception ex)
							 * { ex.printStackTrace(); } }
							 */
						...
						long A = sql.insert("SoundLevel_StnEnq", null, values);
						Log.d("test", "SoundLevel_StnEnq " + values.size());
					}
		...
		protected void onPostExecute(String result) {
			...
			try {
				...
				if (sop.equals("valid")) {
					...
					try{
						ut.showD(StationEnquirySoundLevel.this, "invalid");
					}catch (Exception e){
						e.printStackTrace();
					}
				}
