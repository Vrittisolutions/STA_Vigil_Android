...
import com.adapters.StationEnquiryDownHistoryAdp;
...
public class StationEnquiryDowntimeHistory extends Activity{
	ProgressDialog pd;
	...
	ImageView iv;
    StationEnquiryDownHistoryAdp listAdapter;
	String responsemsg = "k";
	String ActivityName, ActivityId, actname;
	private String mType,mStation,mInstallation;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(com.stavigilmonitoring.R.layout.downtimemain);

		downtime = (ListView) findViewById(com.stavigilmonitoring.R.id.downtimetimelist);
		iv=(ImageView)findViewById(com.stavigilmonitoring.R.id.button_refresh_downtime_main);
		title=(TextView)findViewById(com.stavigilmonitoring.R.id.title);

		intent=getIntent();
		mType=intent.getStringExtra("Network");
		mStation=intent.getStringExtra("stnname");
		mInstallation=intent.getStringExtra("stninst");
		title.setText(mStation+"-Downtime Station");
		...
		if(ut.isnet(StationEnquiryDowntimeHistory.this)){

			fetchdata();
			}
			else {
				try{
					ut.showD(StationEnquiryDowntimeHistory.this,"nonet");
				}catch (Exception e){
					e.printStackTrace();
				}
			}
...
((EditText)findViewById(com.stavigilmonitoring.R.id.edfitertext)).addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				...
				listAdapter.filter(((EditText)findViewById(com.stavigilmonitoring.R.id.edfitertext)).getText().toString().trim().toLowerCase(Locale.getDefault()));
			}
		});
...
//		});
	...
	public void FilterClick(View v)
	{
		...
		{
			((EditText)findViewById(com.stavigilmonitoring.R.id.edfitertext)).setVisibility(View.GONE);
			InputMethodManager imm = (InputMethodManager)getSystemService(INPUT_METHOD_SERVICE);
            imm.hideSoftInputFromWindow(getCurrentFocus().getWindowToken(), 0);
		}
		else 	if(  ((EditText)findViewById(com.stavigilmonitoring.R.id.edfitertext)).getVisibility() == View.GONE )
		{
			((EditText)findViewById(com.stavigilmonitoring.R.id.edfitertext)).setVisibility(View.VISIBLE);
	...
	public void editActivity(String StationName) {
		...
		Cursor c2 = sqldb.rawQuery("SELECT * FROM Downtime where InstallationDesc=? ",
				params);
		String stnname = "";
  ...
  //  startService(new Intent(getBaseContext(),SynchDtataCount.class));
		...
		listAdapter=new StationEnquiryDownHistoryAdp(this, searchResults);
	...
	public ArrayList<String> getDetail()
	{
		...
		Cursor c = sql.rawQuery("Select distinct InstallationDesc from Downtime c1 inner join AllStation c2 on c1.InstalationId=c2.InstallationId where c2.InstallationDesc='"+mStation+"'", null);
...
//
	...
	protected void showD(String string) {
		...
		final Dialog myDialog = new Dialog(StationEnquiryDowntimeHistory.this);
